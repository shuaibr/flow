...
**Domain:** Single-user, local-first, privacy-sensitive wellness/quantified-self system (Garmin ingest + manual logging + readiness signals + scheduled digests)
**Researched:** 2026-06-10
**Confidence:** HIGH overall (each section annotated; verified via official docs, GitHub, PyPI, and web search — not training data alone)
...
Current model ids and pricing (verified 2026-06; use these exact strings — no date suffixes):
...
One weekly call ≈ 10K input + 1.5K output tokens ⇒ ~$0.05/week on Sonnet, ~$0.09/week on Opus. Put `digest_model: claude-sonnet-4-6` and a `max_tokens` cap in `config.yaml`; enforce a monthly token budget in code with alert at 80% (PRINCIPLES §1 Infrastructure capacity). No streaming, batching, or caching machinery needed at this volume.
...
```bash
# Core (uv project)
uv add garminconnect sqlcipher3-wheels fastapi uvicorn jinja2 httpx \
       pydantic pydantic-settings anthropic matplotlib
...
**If the server is a home box / NAS / Raspberry Pi (expected case):**
...
**If you'd rather operate zero web surface:**
...
**If the deploy target is a single always-on container (no systemd):**
...
**LLM budget enforcement (any variant):**
...
---
*Stack research for: Flow — local-first personal wellness OS*
*Researched: 2026-06-10*
